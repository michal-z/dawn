...
DAWN_DECLARE_UNUSED size_t QueueCopyExternalTextureForBrowserGetExtraRequiredSize(const QueueCopyExternalTextureForBrowserCmd& record) {
    ...
    }
    {
        ...
        auto memberLength = 1u;
        result += memberLength * sizeof(WGPUImageCopyExternalTextureTransfer);
        for (decltype(memberLength) i = 0; i < memberLength; ++i) {
...
size_t DeviceForceLossCmd::GetRequiredSize() const {
    size_t size = sizeof(DeviceForceLossTransfer) + DeviceForceLossGetExtraRequiredSize(*this);
    return size;
}
...
size_t QueueCopyExternalTextureForBrowserCmd::GetRequiredSize() const {
    size_t size = sizeof(QueueCopyExternalTextureForBrowserTransfer) + QueueCopyExternalTextureForBrowserGetExtraRequiredSize(*this);
    return size;
}
